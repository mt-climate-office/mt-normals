...
time_avg <- function(
  ...
  times <- tibble::tibble(
    ...
    dplyr::mutate(
      yearly_grp = glue::glue("{season} {year}") %>% factor()
    )
  ...
  if (period == "annual"){
    ...
    out <- terra::tapp(
      r_filtered, index = times$yearly_grp, fun = fun
    )  
  } else {
    out <- r_filtered
  }
...
rast_df <- tidyr::crossing(
  ...
  reference_period = list(c(1950, 2014), reference_period, mid_century, end_century)
) %>%
  dplyr::mutate(
