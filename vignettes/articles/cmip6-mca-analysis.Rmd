...
filter_and_read <- function(pattern, shp, attr_id, historical, period, fun="mean", zonal=TRUE, timescale="annual") {
  ...
  purrr::map(function(x) {
      ...
      out <- spat_summary(
        tmp, shp = shp, attr_id = attr_id, fun="mean"
      ) %>% 
        dplyr::select(div = NAME, year = timescale, value) %>% 
        dplyr::mutate(
          f = basename(x) %>% tools::file_path_sans_ext(),
          period = paste(period, collapse = "-")
        )
...
make_heatmap_data <- function(pattern, fun) {
  dat <- dplyr::bind_rows(
    filter_and_read(pattern=pattern, historical=TRUE, period=c(1981, 2010), fun=fun, TRUE, "monthly"),
    filter_and_read(pattern=pattern, historical=FALSE, period=c(2049, 2069), fun=fun, TRUE, "monthly"),
    filter_and_read(pattern=pattern, historicalFALSE, period=c(2070, 2099), fun=fun, TRUE, "monthly") 
  ) %>%
    tidyr::separate(
      f, c("model", "scenario", "run", "variable"), sep = "_"
    ) %>%
    dplyr::mutate(date = lubridate::as_date(year, format = "X%Y.%m.%d")) %>%
    dplyr::group_by(div, month = lubridate::month(date), scenario, period) %>% 
    ...
    dplyr::select(div, month, scenario, diff, period) %>%
    dplyr::mutate(
      div = tolower(div) %>% tools::toTitleCase(),
...
ggplot(dat) +
  geom_tile(aes(x=month, y=div, fill=diff), color="black") + 
...
dat <- make_heatmap_data("tas.tif", "mean")
