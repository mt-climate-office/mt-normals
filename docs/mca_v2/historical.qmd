...
rasts <- list.files(params$asset_dir, pattern = "historical_seasonal.tif", full.names = T) %>%
  tibble::tibble(f = .) %>% 
  dplyr::rowwise() %>% 
...
historical_table <- 
  purrr::map(rasts$f, function(x) {
    ...
    dplyr::mutate(
      value = ifelse(variable == "pr", value / 25.4, (value - 273.15) * 9/5 + 32)
    )

global_avg <- rasts %>% 
  dplyr::mutate(r = list(terra::global(r, fun="mean") %>%
...
### Temperature
@tbl-historical-temp shows the average seasonal temperature variation across `r params$location`’s `r nrow(shp)` `r params$attr_plural` (Figure 2-3) from 1981-2010. Temperatures vary across `r params$location`, with average annual values ranging from `r min(historical_table$value) %>% round(1)`°F to `r max(historical_table$value) %>% round(1)`°F across the region. 

```{r echo=FALSE}
#| label: tbl-historical-temp
#| tbl-cap: "Average temperatures (°F) for the seven Montana climate divisions from 1981-2010."
historical_table %>% 
