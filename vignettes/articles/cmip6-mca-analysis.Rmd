...
filter_and_read <- function(pattern, historical, period, fun="mean", zonal=TRUE) {
  ...
  purrr::map(function(x) {
      ...
      if (!zonal) {
        tmp <- timeseries_to_summary(tmp, "annual")
...
isolate_models <- function(pattern, historical, period, fun="mean") {
  filter_and_read(pattern, historical, period, fun, TRUE) %>% 
  dplyr::bind_rows() %>% 
  dplyr::mutate(
    div = tolower(div),
...
dat <- dplyr::bind_rows(
  isolate_models("tas.tif", TRUE, c(1981, 2010), "mean"),
  isolate_models("tas.tif", FALSE, c(2049, 2069), "mean"),
  isolate_models("tas.tif", FALSE, c(2070, 2099), "mean")
)
...
dat %>% 
  ...
  ggplot(aes(x=scenario, y=diff, fill=scenario)) + 
...
dat %>% 
  ...
  dplyr::filter(scenario != "historical") %>%
  dplyr::rowwise() %>%
  dplyr::mutate(diff = list(r - historical)) -> test
```
