...
```{r make_tmp_data}
tavg_box_dat <- normals::make_boxplot_data(
  files=cmip_files, pattern="tas.tif", shp=shp, attr_id=params$attr_id, 
  fun="mean", tsfm = tas_tsfm)
```
...
normals::make_boxplot_plot(tavg_box_dat, "Temperature Change (°F)", "Change in Mean Annual Temperature")
```
