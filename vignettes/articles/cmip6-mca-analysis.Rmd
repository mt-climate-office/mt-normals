...
tmmn <- "https://data.climate.umt.edu/mca/gridmet/tmmn.tif"
tmmx <- "https://data.climate.umt.edu/mca/gridmet/tmmx.tif"
pr <- "https://data.climate.umt.edu/mca/gridmet/pr.tif"

gm_df <- tidyr::crossing(
...
gm_df %>% 
  ...
  dplyr::mutate(
    variable = dplyr::recode(
      variable,
      "tmmx" = "Avg. Max Temperature",
      "tmmn" = "Avg. Min Temperature",
      "pr" = "Total Precipitation"
    ),
    value = round(value)
  ) %>%
...
plt <- gm_df %>% 
  ...
  dplyr::rowwise() %>% 
  dplyr::mutate(dat = list(to_shp(r_summarized, mt=mt))) %>% 
  dplyr::select(variable, dat) %>%
  dplyr::group_split() %>%
